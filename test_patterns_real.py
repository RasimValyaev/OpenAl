...
from nomenclature_extract_pattern_optimized import nomenclature_pattern, extract_float
...
for text in test_cases:
    ...
    if result:
        ...
        # If there are 3 parameters and second param > 1, use "box" instead of "шт"
        unit = "box" if boxes > 1 and pieces > 1 else "шт"
        print(f"✓ {text}")
        print(f"  -> {weight}г x {pieces}{unit} x {boxes}")
    else:
        print(f"✗ {text}")
        print("  -> No match")